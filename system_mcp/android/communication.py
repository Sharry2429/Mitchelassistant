...
def get_call_history(limit: int=50) -> MCPResult:
    ...
    try:
        output = adb.shell('content query --uri content://call_log/calls --projection number:type:duration:date')
        ...
        for line in output.splitlines():
            ...
            for part in parts:
                if '=' in part:
                    ...
                    if k == 'type':
                        v_int = int(v)
                        entry['type'] = 'INCOMING' if v_int == 1 else 'OUTGOING' if v_int == 2 else 'MISSED' if v_int == 3 else str(v_int)
                    else:
                        entry[k] = v
        ...
        return MCPResult.fail(str(e))
'\nAndroid SMS management via ADB content provider queries.\nNote: Sending SMS requires the companion app or a direct ADB workaround.\n'
...
def read(limit: int=10, offset: int=0) -> MCPResult:
    ...
    try:
        result = adb.shell(['content', 'query', '--uri', 'content://sms/inbox', '--projection', 'address:body:date', '--sort', f'date DESC LIMIT {limit} OFFSET {offset}'])
        messages = []
        if result:
...
def send(phone_number: str, message: str) -> MCPResult:
    ...
    try:
        adb.shell(['am', 'start', '-a', 'android.intent.action.SENDTO', '-d', f'sms:{phone_number}', '--es', 'sms_body', message, '--ez', 'exit_on_sent', 'true'])
...
def list_contacts() -> MCPResult:
    ...
    try:
        result = adb.shell(['content', 'query', '--uri', 'content://contacts/phones/', '--projection', '_id:display_name:number'])
        contacts = []
        if result:
            for line in result.strip().splitlines():
                if 'Row:' in line:
...
def get_contact(contact_id: str) -> MCPResult:
    ...
    try:
        result = adb.shell(['content', 'query', '--uri', f'content://contacts/phones/{contact_id}', '--projection', '_id:display_name:number'])
        if result:
            for line in result.strip().splitlines():
                if 'Row:' in line:
                    parts = {}
...
def add_contact(name: str, phone: str) -> MCPResult:
    ...
    try:
        result = adb.shell(['am', 'start', '-a', 'android.intent.action.INSERT', '-t', 'vnd.android.cursor.dir/contact', '--es', 'name', name, '--es', 'phone', phone])
...
def delete_contact(contact_id: str) -> MCPResult:
    ...
    try:
        adb.shell(['content', 'delete', '--uri', f'content://contacts/phones/{contact_id}'])
        return MCPResult.success(None)
    except Exception as e:
        return MCPResult.fail(str(e))
'\nsystem_mcp.android.whatsapp\nWhatsApp messaging and reading using Intent + Notification Listener strategy.\n'
...
def send_message(phone_number: str, text: str) -> MCPResult:
    ...
    try:
        ...
        intent_url = f'https://api.whatsapp.com/send?phone={phone_number}&text={encoded_text}'
        output = adb.shell(f"am start -a android.intent.action.VIEW -d '{intent_url}' com.whatsapp")
        if 'Error' in output or 'Exception' in output:
            return MCPResult.fail(f'Failed to launch WhatsApp: {output}')
        return MCPResult.success(f'WhatsApp opened for {phone_number} with pre-filled message.')
    except Exception as e:
        return MCPResult.fail(str(e))
    ...
    try:
        ...
        for n in notifs_result.data:
            if n.get('packageName') == 'com.whatsapp':
                chat = {'title': n.get('title', 'Unknown'), 'text': n.get('text', ''), 'postTime': n.get('postTime', 0)}
    ...
    try:
        ...
        for c in res.data:
            ...
            if target_name in c_name:
                return MCPResult.success({'name': c.get('display_name'), 'number': c.get('number')})
...
def start_voice_call(contact_name_or_number: str) -> MCPResult:
    ...
    try:
        ...
        return MCPResult.fail('Accessibility approach deferred as per best practices. Only intent sending is supported.')
    except Exception as e:
        return MCPResult.fail(str(e))
...
def list_events(start_time: str=None, end_time: str=None):
    ...
    try:
        result = adb.shell(['content', 'query', '--uri', 'content://com.android.calendar/events', '--projection', '_id:title:dtstart:dtend:eventLocation'])
...
def add_event(event_data: dict):
    ...
    try:
        ...
        result = adb.shell(['am', 'start', '-a', 'android.intent.action.INSERT', '-t', 'vnd.android.cursor.item/event', '--es', 'title', title, '--el', 'beginTime', str(beginTime), '--el', 'endTime', str(endTime)])
        data = result.stdout if hasattr(result, 'stdout') else result
        return MCPResult.success(data)
    except Exception as e:
        return MCPResult.fail(str(e))
...
def delete_event(event_id: str):
    ...
    try:
        result = adb.shell(['content', 'delete', '--uri', 'content://com.android.calendar/events', '--where', f'_id={event_id}'])
        data = result.stdout if hasattr(result, 'stdout') else result
        return MCPResult.success(data)
    except Exception as e:
        return MCPResult.fail(str(e))
