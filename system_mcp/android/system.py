...
def reboot(confirm: bool=False):
    ...
    try:
        ...
        if hasattr(result, 'stderr') and result.stderr and ('Permission denied' in result.stderr):
...
def stay_awake(enable: bool):
    ...
    try:
        ...
        if hasattr(result, 'stderr') and result.stderr and ('Permission denied' in result.stderr or 'Killed' in result.stderr):
...
def kill(process_name: str, confirm: bool=False):
    ...
    try:
        ...
        if hasattr(result, 'stderr') and result.stderr and ('Permission denied' in result.stderr or 'Operation not permitted' in result.stderr):
            return MCPResult.fail(result.stderr)
        data = result.stdout if hasattr(result, 'stdout') else result
        return MCPResult.success(data)
    except Exception as e:
...
def storage():
    ...
    try:
        ...
        return MCPResult.fail(str(e))
'\nAndroid settings management.\nReads settings via ADB, writes via Companion APK (WRITE_SECURE_SETTINGS).\n'
...
def grant(package_name: str, permission: str) -> MCPResult:
    ...
    log_action('permissions', 'grant', {'package': package_name, 'permission': permission}, {})
    try:
        result = adb.shell(['pm', 'grant', package_name, permission])
        if result.returncode != 0:
            return MCPResult.fail(f'Failed to grant {permission}: {result.stderr.strip()}')
        return MCPResult.success(None)
    except SystemMCPError as e:
        return MCPResult.fail(str(e))
...
def revoke(package_name: str, permission: str) -> MCPResult:
    ...
    log_action('permissions', 'revoke', {'package': package_name, 'permission': permission}, {})
    try:
        result = adb.shell(['pm', 'revoke', package_name, permission])
        if result.returncode != 0:
            return MCPResult.fail(f'Failed to revoke {permission}: {result.stderr.strip()}')
        return MCPResult.success(None)
    except SystemMCPError as e:
        return MCPResult.fail(str(e))