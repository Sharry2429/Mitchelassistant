...
def screenshot(region: Optional[Tuple[int, int, int, int]] = None, save_path: Optional[str] = None) -> ScreenshotResult:
...
def find_element(text: str = '', control_type: str = '') -> Optional[UIElement]:
    ...
    for el in elements:
        ...
        match_ctype = True
        if control_type:
            match_ctype = (control_type.lower() in el.control_type.lower()) if el.control_type else False
...
__all__ = [
    'click', 'double_click', 'right_click', 'middle_click', 'move_mouse',
    'drag', 'scroll', 'get_mouse_position',
...
def wait_for(condition: str, text: Optional[str] = None, timeout: float = 10.0, poll_interval: float = 0.5) -> bool:
    if condition == 'text_exists':
        from system_mcp.windows.ocr import get_desktop_text
        ...
        from system_mcp.windows.window import get_active_window_title
        start_time = time.time()
        while time.time() - start_time < timeout:
        ...
        from system_mcp.windows.uia import element_exists
        start_time = time.time()
        while time.time() - start_time < timeout:
...
def set_clipboard_image(image_path: str) -> None:
    ...
    try:
        ...
        win32clipboard.SetClipboardData(win32con.CF_DIB, image_data[14:]) # Skip BMP header if assuming it's BMP
    finally:
        win32clipboard.CloseClipboard()
...
def send_notification(title: str, body: str, icon_path: Optional[str] = None, duration: str = 'short') -> bool:
