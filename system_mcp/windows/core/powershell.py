...
def get_powershell_path() -> str:
    ...
    try:
        ...
        result = subprocess.run(["where", "pwsh"], capture_output=True, text=True, check=False)
...
def _prepare_env() -> Dict[str, str]:
    ...
    sys_env = _get_registry_env(winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\CurrentControlSet\Control\Session Manager\Environment")
...
def _run_with_timeout(args: List[str], timeout: Optional[int], env: Dict[str, str], requested_encoding: Optional[str] = None) -> CommandResult:
    try:
...
def execute(command: str, timeout: Optional[int] = None, as_admin: bool = False, encoding: Optional[str] = None) -> CommandResult:
    config = get_config()
    # Use provided timeout or fallback to config, if it is None
    actual_timeout = timeout if timeout is not None else getattr(config, 'default_timeout', 60)
    ...
    args = [ps_path, "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-EncodedCommand", encoded_command]
    env = _prepare_env()
    
    return _run_with_timeout(args, actual_timeout, env, requested_encoding=encoding)

def execute_script(script_path: str, args: Optional[List[str]] = None, timeout: Optional[int] = None) -> CommandResult:
    config = get_config()
    actual_timeout = timeout if timeout is not None else getattr(config, 'default_timeout', 60)
    
    ps_path = get_powershell_path()
    cmd_args = [ps_path, "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", script_path]
