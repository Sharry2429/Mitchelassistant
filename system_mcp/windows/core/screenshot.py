...
def get_monitor_rects() -> List[Tuple[int, int, int, int]]:
    ...
    @ctypes.WINFUNCTYPE(ctypes.wintypes.BOOL, ctypes.wintypes.HANDLE, ctypes.wintypes.HANDLE, ctypes.POINTER(ctypes.wintypes.RECT), ctypes.wintypes.LPARAM)
...
def capture_screen_gdi(region: Optional[Tuple[int, int, int, int]] = None) -> Any:
    ...
    gdi32.GetDIBits(mem_dc, screenshot_bmp, 0, height, buffer, ctypes.byref(bmi), DIB_RGB_COLORS)
...
def capture_screen_dxcam(region: Optional[Tuple[int, int, int, int]] = None, monitor: int = 0) -> Any:
...
def capture_screen(region: Optional[Tuple[int, int, int, int]] = None, monitor: Optional[int] = None) -> Any:
    """
    Captures the screen based on configuration (DXCam or GDI).
    Falls back to GDI if DXCam is configured but unavailable.
    """
    config = get_config()
    use_dxcam = config.get("use_dxcam", False) if isinstance(config, dict) else getattr(config, 'use_dxcam', False)
