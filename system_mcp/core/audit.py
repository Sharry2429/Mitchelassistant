...
DESTRUCTIVE = ["uninstall", "clear_data", "delete", "reboot", "shutdown", "revoke", "kill", "force-stop"]
SENSITIVE_READ = ["notification.stream", "sms.read", "contacts.", "calendar.", "clipboard.read", "whatsapp.", "phone.", "assistant.", "overlay."]
...
def log_action(module: str, action: str, args: Dict[str, Any], kwargs: Dict[str, Any], caller: str = "system"):
...
def check_destructive(action: str, confirm: bool):
    ...
    if get_config().safeguards:
        if action in DESTRUCTIVE and not confirm:
            raise RequiresConfirmation(f"Action '{action}' is destructive. Pass confirm=True to execute.")
