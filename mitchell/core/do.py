...
mitchell.core.do
================
The ONE Mitchell interface. Just tell it — it routes everything.

    mitchell-do "open example.com and tell me the title"   # one-shot
    mitchell-do                                            # chat / REPL
    mitchell-do "check the android battery"
    mitchell-do "implement a fibonacci function in fib.py"
...
Routing (automatic, no prompts):
  ...
  * browser / Windows / Android            -> lean agent loop, starting on the
    ...
    So everyday chat is fast+cheap; harder tasks pull more power on demand.

Nothing faked; wall-clock measured throughout.
"""
...
def _print(res: dict) -> None:
    print(f"[{res.get('kind', '?')}] elapsed={res.get('elapsed', '?')}s")
    ans = res.get("answer")
    if ans:
        print(f"\n{ans}")
    elif res.get("error"):
    ...
    detail = ""
    if res.get("steps_ok") is not None:
        detail = f" state={res.get('state')} steps={res.get('steps')} (ok={res.get('steps_ok')})"
    if detail:
...
def main() -> int:
    ...
    if args:
        t0 = time.monotonic()
        res = asyncio.get_event_loop().run_until_complete(route_one(args))
        _print(res)
        print(f"\n  total {round(time.monotonic() - t0, 2)}s")
        return 0 if res.get("ok") else 1

    # REPL: the single chat interface.
    print("Mitchell — just tell it. (`exit` to quit)")
    ...
    while True:
        ...
        if not line or line.lower() in ("exit", "quit"):
            break
        res = loop.run_until_complete(route_one(line))
        _print(res)
    return 0
