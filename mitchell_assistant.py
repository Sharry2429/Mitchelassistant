...
def print_telemetry(model, duration, usage):
    ...
    if model in COSTS_USD:
        cost = (in_t * COSTS_USD[model]["in"] + out_t * COSTS_USD[model]["out"]) * INR_RATE
    print(f"\033[90m[⏱ Time: {duration:.1f}s | 🪙 Tokens: {total} | 💸 Cost: ₹{cost:.4f}]\033[0m")
...
async def main():
    ...
    while True:
        try:
            ...
            end_t = time.time()
            print_telemetry("openai/gpt-5.6-luna", end_t - start_t, router_response.usage)
